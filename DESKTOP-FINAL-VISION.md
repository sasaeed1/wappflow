...
| P7 | CC **Desktop Fleet page** (`/control/desktop`) — versions, machine count, devices, version-policy editor | 🌐 | ✅ | `next build` ✓ |
| P7 | CC **UI depth** remainder — plans editor, flags rollout UI, user mgmt + MFA, billing/Stripe, impersonation UI, SQL console frontend, system health | 🌐 | ⏸ | control/* pages exist from P1; need depth |
...
| P10 | **Storage abstraction** (`storage/` providers/local+r2, signed-urls, index) + `STORAGE_PROVIDER` + single `publicUrl()` + Media Studio integration + per-asset tracking (provider/size/uploaded_at) + **signed upload URLs** (desktop direct-to-R2) + provider-aware ingest + provider-aware delete | 🌐 | ✅ | verified `test-storage.js` + ingest-path check; `next build` ✓ |
| P10 | **CC Storage Dashboard** (`/control/storage`) — global R2 usage/cost/growth + top-20 workspaces + per-workspace drilldown | 🌐 | ✅ | verified `test-cc-storage.js` + build ✓ |
| P10 | R2 remainder — **new uploads + variants → R2** ✅ done; remaining: album-PDF + RAW-preview + watermark on R2 · optional one-time backfill script · custom CDN (`R2_PUBLIC_BASE`, deferred per spec) | 🌐 | 🟡 | needs `STORAGE_PROVIDER=r2` on host to activate |
