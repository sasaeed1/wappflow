...
// Contracts is its own module (opened from the app-switcher, in its own tab),
// so it gets its own shell instead of the CRM NavBar — same pattern as Studio.
export default function ContractsShell({ children }) {
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      <header style={{ position: 'sticky', top: 0, zIndex: 100, height: 58, display: 'flex', alignItems: 'center', gap: 8, padding: '0 18px', background: 'var(--surface)', borderBottom: '1px solid var(--border)', boxShadow: '0 1px 8px rgba(0,0,0,0.06)' }}>
        ...
        <div onClick={() => router.push('/contracts')} style={{ display: 'flex', alignItems: 'center', gap: 9, cursor: 'pointer', marginRight: 6 }}>
          ...
          <b style={{ fontSize: 15.5, color: 'var(--text)', letterSpacing: '-0.3px' }}>Contracts</b>
        </div>
        <nav style={{ display: 'flex', gap: 2, marginLeft: 6 }}>
          <NavLink href="/contracts" label="Contracts" />
          <NavLink href="/contracts/templates" label="Templates" />
        </nav>
        <div style={{ flex: 1 }} />
