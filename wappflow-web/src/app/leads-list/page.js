...
import { leadsAPI, tagsAPI, workspaceAPI, displayPhone } from '../../lib/api';
import NavBar from '../../components/NavBar';
import { TagChip, TagPicker } from '../../components/TagPicker';
import AddLeadModal from '../../components/AddLeadModal';

const STATUS_META = {
  'New':           { dot: '#6366f1', bg: '#eef2ff',  text: '#4338ca' },
  'Contacted':     { dot: '#06b6d4', bg: '#ecfeff',  text: '#0e7490' },
  'Interested':    { dot: '#f59e0b', bg: '#fffbeb',  text: '#b45309' },
  'Negotiating':   { dot: '#f97316', bg: '#fff7ed',  text: '#c2410c' },
  'Closed - Won':  { dot: '#10b981', bg: '#ecfdf5',  text: '#047857' },
  'Closed - Lost': { dot: '#ef4444', bg: '#fef2f2',  text: '#b91c1c' },
};
        ...
        {mode === 'manual' ? (
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, maxHeight: 240, overflowY: 'auto' }}>
              <button
                onClick={() => setSelected('')}
                style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', borderRadius: 10, border: `1.5px solid ${selected === '' ? '#6366f1' : 'var(--border)'}`, background: selected === '' ? '#eef2ff' : 'var(--surface2)', cursor: 'pointer', textAlign: 'left' }}
              >
              ...
              {members.map(m => {
                ...
                return (
                  <button key={m.id} onClick={() => setSelected(m.user_id || m.id)} style={{
                    ...
                    background: selected === (m.user_id || m.id) ? '#eef2ff' : 'var(--surface2)', cursor: 'pointer', textAlign: 'left'
                  }}>
              ...
              {activeMembers.length === 0 ? (
                ...
                return (
                  <button key={userId} onClick={() => toggleRr(userId)} style={{
                    ...
                    background: isOn ? '#eef2ff' : 'var(--surface2)', cursor: 'pointer', textAlign: 'left',
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      ...
      <div style={{ background: 'var(--surface)', borderBottom: '1px solid var(--border)', padding: '14px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <span style={{ fontSize: 17, fontWeight: 800, color: 'var(--text)' }}>All Leads</span>
          <span style={{ fontSize: 11, fontWeight: 700, color: '#6366f1', background: '#eef2ff', border: '1px solid #c7d2fe', padding: '2px 10px', borderRadius: 20 }}>{allLeads.length}</span>
        </div>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          ...
          <button
            ...
            style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 14px', background: leads.length ? '#ecfdf5' : 'var(--surface2)', border: `1.5px solid ${leads.length ? '#a7f3d0' : 'var(--border)'}`, borderRadius: 10, color: leads.length ? '#059669' : 'var(--text-dim)', fontSize: 13, fontWeight: 700, cursor: leads.length ? 'pointer' : 'not-allowed' }}
          >
      ...
      <main style={{ maxWidth: 1280, margin: '0 auto', padding: '20px 24px' }}>
        ...
        <div style={{ background: 'var(--surface)', border: '1.5px solid var(--border)', borderRadius: 16, padding: '12px 16px', marginBottom: 14, display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', boxShadow: '0 1px 4px rgba(0,0,0,0.05)' }}>
          ...
          {/* Date range toggle */}
          <button onClick={() => setShowFilters(v => !v)} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 12px', border: `1.5px solid ${hasFilters ? '#c7d2fe' : 'var(--border)'}`, borderRadius: 10, background: hasFilters ? '#eef2ff' : 'var(--surface)', color: hasFilters ? '#6366f1' : 'var(--text-muted)', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
        ...
        {allTags.length > 0 && (
          <div style={{ display: 'flex', gap: 8, marginBottom: 12, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            <button onClick={() => setTagFilter(null)} style={{ padding: '4px 12px', borderRadius: 20, border: `1.5px solid ${!tagFilter ? '#6366f1' : 'var(--border)'}`, background: !tagFilter ? '#eef2ff' : 'var(--surface)', color: !tagFilter ? '#4338ca' : 'var(--text-muted)', fontSize: 11, fontWeight: 700, cursor: 'pointer' }}>All</button>
            {allTags.map(tag => (
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 16, overflowX: 'auto', paddingBottom: 4 }}>
          {ALL_STATUSES.map(s => {
            const meta = STATUS_META[s] || { dot: '#6366f1', bg: '#eef2ff', text: '#4338ca' };
        ...
        <div style={{ background: 'var(--surface)', border: '1.5px solid var(--border)', borderRadius: 16, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.05)' }}>
          ...
          {loading ? (
            ...
            return (
              <div key={lead.id} style={{ display: 'grid', gridTemplateColumns: '40px 2fr 1.2fr 1fr 1fr 1.4fr 1fr 1fr 40px', alignItems: 'center', padding: '12px 16px', borderBottom: i < leads.length-1 ? '1px solid var(--border)' : 'none', background: isSelected ? '#eef2ff' : 'var(--surface)', transition: 'background 0.1s', cursor: 'pointer' }}
              ...
              >
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <div>
                    <p style={{ fontSize: 14, fontWeight: 700, color: 'var(--text)', margin: 0 }}>{lead.customer_name || 'Unknown'}</p>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                  ...
                  <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>{displayPhone(lead.customer_phone)}</span>
                </div>
                ...
                <div onClick={e => e.stopPropagation()}>
                  {assignedName ? (
                    <span style={{ fontSize: 11, fontWeight: 600, padding: '3px 9px', borderRadius: 20, background: '#eef2ff', color: '#6366f1', display: 'inline-flex', alignItems: 'center', gap: 4 }}>
