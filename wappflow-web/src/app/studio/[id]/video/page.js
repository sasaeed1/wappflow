              ...
              {selected && (
                <>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', margin: '14px 0' }}>
                    ...
                    <button onClick={addClip} disabled={markIn == null || markOut == null || markOut <= markIn} style={{ padding: '8px 14px', borderRadius: 9, border: 'none', cursor: 'pointer', background: (markIn == null || markOut == null || markOut <= markIn) ? 'var(--surface2)' : 'linear-gradient(135deg,#ec4899,#8b5cf6)', color: '#fff', fontWeight: 800, fontSize: 12.5, opacity: (markIn == null || markOut == null || markOut <= markIn) ? 0.6 : 1, display: 'flex', alignItems: 'center', gap: 5 }}><Scissors size={13} /> Add clip</button>
                  </div>
